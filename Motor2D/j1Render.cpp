...
void j1Render::manageCameraMovement(float dt)
{
	int mov_x, mov_y;
	mov_x = mov_y = 0;

	iPoint world_size = App->map->MapToWorld({ App->map->worldData.world_width, App->map->worldData.world_height });

	if (App->input->GetKey(SDL_SCANCODE_LEFT) == KEY_REPEAT && camera.x > 0)
		mov_x = -CAMERA_SPEED;  //Move left
	else if (App->input->GetKey(SDL_SCANCODE_RIGHT) == KEY_REPEAT && camera.x < world_size.x)
		mov_x = CAMERA_SPEED; //Move right

	if (App->input->GetKey(SDL_SCANCODE_UP) == KEY_REPEAT && camera.y > 0)
		mov_y = -CAMERA_SPEED;	 //Move up 
	else if (App->input->GetKey(SDL_SCANCODE_DOWN) == KEY_REPEAT && camera.y < world_size.y)
		mov_y = CAMERA_SPEED; //Move down

	camera.x += mov_x * dt;
	camera.y += mov_y * dt;

	camera.x = (camera.x > world_size.x ? world_size.x : (camera.x < 0 ? 0 : camera.x));
	camera.y = (camera.y > world_size.y ? world_size.y : (camera.y < 0 ? 0 : camera.y));
}

bool j1Render::collisionWithCamera(SDL_Rect rect)
{
	return (rect.x - CULLING_MARGIN < camera.x + camera.w &&
		rect.x + rect.w > camera.x - CULLING_MARGIN &&
		rect.y - CULLING_MARGIN< camera.y + camera.h &&
		rect.y + rect.h > camera.y - CULLING_MARGIN
		);
}
