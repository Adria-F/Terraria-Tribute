...
bool j1Input::collidingMouse(SDL_Rect rect, bool use_camera)
{
	if (use_camera)
		return (mouse_x + App->render->camera.x > rect.x && (mouse_x + App->render->camera.x) < (rect.x + rect.w) && (mouse_y + App->render->camera.y) > rect.y && (mouse_y + App->render->camera.y) < (rect.y + rect.h));
	else
